resource "vultr_kubernetes" "vke_np_cluster" {
  ...
  #    node_quantity = 1
  #    plan          = "vc2-1c-2gb"
  #    label         = "vke-default-np-node-pool"
  #    auto_scaler   = false
  #    min_nodes     = 1
  #    max_nodes     = 1
  #  }
...
resource "vultr_kubernetes_node_pools" "np_node_pool_1" {
  cluster_id    = vultr_kubernetes.vke_np_cluster.id
  node_quantity = 1
  plan          = "vc2-1c-2gb"
  label         = "vke-np-node-pool-1"
  auto_scaler   = false
  min_nodes     = 1
  max_nodes     = 1
}
...
resource "vultr_kubernetes_node_pools" "np_node_pool_2" {
  cluster_id    = vultr_kubernetes.vke_np_cluster.id
  node_quantity = 1
  plan          = "vc2-1c-2gb"
  label         = "vke-np-node-pool-2"
  auto_scaler   = false
  min_nodes     = 1
  max_nodes     = 1
}
