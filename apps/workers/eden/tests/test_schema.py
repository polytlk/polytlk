...
hanji_word_st = st.characters(min_codepoint=0x4E00, max_codepoint=0x9FFF).filter(is_cjk)  # noqa: WPS432, E501

pinyin_sent_st = st.from_regex(pinyin_sent, fullmatch=True)

# WORDS
good_words_st = lists(
    tuples(hanji_word_st, pinyin_word_st, text(min_size=1)),
    min_size=1,
    max_size=1,
)
...
# DIALOGUE
good_dialogue_st = lists(
    tuples(hanji_word_st, pinyin_sent_st, text(min_size=1)),
    min_size=1,
    max_size=1,
)
...
@pytest.mark.words
@given(
    words=lists(bad_word_pinyin_st, min_size=1, max_size=1),
    dialogue=good_dialogue_st,
    meaning=text(min_size=1),
)
...
@pytest.mark.words
@given(
    words=lists(bad_word_hanji_st, min_size=1),
    dialogue=good_dialogue_st,
    meaning=text(min_size=1),
)
...
@given(
    words=good_words_st,
    dialogue=lists(bad_dialogue_pinyin_st, min_size=1),
    meaning=text(min_size=1),
)
