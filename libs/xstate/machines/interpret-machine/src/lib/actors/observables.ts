...
export const interpretation$ = fromCallback<EventObject, { baseUrl: string, taskId: string, token: string }>(({ sendBack, input }) => {
    const eventSource = new EventSource(`${input.baseUrl}/api/chinese/task/${input.taskId}/stream?key=${input.token}`);
