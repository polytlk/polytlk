...
VALID_TARGETS=("cert-manager" "nginx-ingress" "external-secrets" "otel-collector")
...
helmfile template -e $ENVIRONMENT -l type=secret -f "helm/base/$TARGET/helmfile.yaml.gotmpl" --output-dir "../../../$TARGET_FOLDER" --output-dir-template "{{ .OutputDir }}/{{ .Release.Name }}"
